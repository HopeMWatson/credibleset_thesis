...
pvalues <- p 
splitp <- split(p, rep(1:ncol(p), each = nrow(p)))

masplit <- split(p, col(p)) 
print(masplit)


cSplit(p, splitCols = p, sep = 100)
help("cSplit")
...
for (i in p)
{
  my.res <- finemap.abf(dataset=list(pvalues=masplit$1:100, N=1000, MAF=MAF, s=0.5, type="cc"), p1=1e-04)
}

MAF <- colMeans(haps)

my.res <- finemap.abf(dataset=list(pvalues=masplit$1, N=1000, MAF=MAF, s=0.5, type="cc"), p1=1e-04)
