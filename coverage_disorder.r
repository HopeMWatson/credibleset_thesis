...
simdata <- function(freq,OR=1.5,nrep=10,N0=1000,N1=1000) {
    ...
    results <- lapply(1:nrep, function(i) {
        tmp <- subset(finemap.abf(dataset=list(pvalues=p[i,], N=N0+N1, MAF=MAF, s=N1/(N0+N1), type="cc"), p1=1e-04),snp!="null")
...
## return individual coverage and entropy for nominal cred set size of 0.5 by default
wrapper2 <- function(thr=0.5,...) {
    ...
    cs <- do.call("rbind",cs)
    ent <- sapply(data, function(x) entropy(x$pvalues))
...
## STUFF ABOUT COVERAGE CHANGING WITH SCENARIOS
...
## STUFF ABOUT COVERAGE AND ENTROPY
...
## fake entropy
entropy <- function(p) -sum(log(p))


covent <- replicate(100,wrapper2(), simplify=FALSE)
covent <- do.call("rbind", covent) # smush 'em together
covent$covered <- as.numeric(covent$covered)


## your task today is to learn relationships in covent

library(ggplot2)
ggplot(covent,aes(x=entropy,y=nvar)) + geom_point() + geom_smooth() + facet_wrap(~order)


## questions - as number of snps goes up, does sum(-log(p)) also go up?
## do this by changing nsnps from 100 to 200 in simref()
                                        

ggplot(covent, aes(x=entropy,y=as.numeric(covered))) + geom_point() + geom_smooth(method="lm") + facet_wrap(~order)

## logistic regression
m <- glm(covered ~ entropy, data=covent[ covent$order==TRUE, ], family="binomial")
summary(m)

m <- glm(covered ~ entropy, data=covent[ covent$order==FALSE, ], family="binomial")
summary(m)
